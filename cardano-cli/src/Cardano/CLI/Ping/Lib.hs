...
import           Control.Monad (Monad (..), replicateM, unless, when)
      ...
      case msg of
        LogMsg bs -> do
          let bs' = case (json, first) of
                (True, False)  -> LBSC.pack ",\n" <> bs
                (True, True)   -> LBSC.pack "{ \"pongs\": [ " <> bs
                (False, True)  -> LBSC.pack "   timestamp,                         host,                          cookie,  sample,  median,     p90,    mean,     min,     max,     std\n" <> bs
                (False, False) -> bs
