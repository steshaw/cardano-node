...
hprop_leadershipSchedule = integration . H.runFinallies . H.workspace "alonzo" $ \tempAbsBasePath' -> do
  ...
  assertByDeadlineMCustom "stdout does not contain \"until genesis start time\"" deadline $ do
  ...
  assertByDeadlineMCustom "Leader schedule is correct" leadershipDeadline $ do
    leaderSlots <- getRelevantLeaderSlots (nodeStdout $ poolRuntime poolNode1) (minimum expectedLeadershipSlotNumbers)
