...
nptraversals :: forall r w xss. (Arrow r, ArrowApply w, All SListI xss) => NP (CTraversals r w xss) xss
nptraversals = case sList :: SList xss of
  ...
  SCons ->
    (CTraversals
      ...
      :: (All SListI xss', SListI xs) =>       CTraversals r w (xs ': xss') xs)
    :*
    (hliftA
     ((\(CTraversals x) -> CTraversals (x . tail'))
      :: (All SListI xss', SListI xs)
      => CTraversals r w        xss'  x
      -> CTraversals r w (xs ': xss') x)
     (nptraversals :: (All SListI xss') => NP (CTraversals r w        xss')  xss')
     :: (All SListI xss', SListI xs) =>    NP (CTraversals r w (xs ': xss')) xss')

tail' :: (Arrow r, ArrowApply w) => GTraversal r w (NS (NP I) (xs ': xss)) (NS (NP I) xss)
